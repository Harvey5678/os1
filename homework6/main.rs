///This is the homework6 code
use std::env; // to get arugments passed to the program
use std::thread;  //to create thread
use std::thread::JoinHandle;  //to join thread
...
fn main() {
    ...
    //clone vectors that I do not need to care about ownership later
    let v1 = xs[0].clone();
    let v2 = xs[1].clone();
    ...
    //join two threads and store return value in r1 and r2
    let r1 = t1.join().unwrap();
    let r2 = t2.join().unwrap();

    //gather intermediate sums
    ...
    //store partition data into a vector
    let xss = partition_data(num_partitions, &v);

    // Print info about the partitions
    print_partition_info(&xss);

    //store joinhandler in vector tp
    let mut tp : Vec<JoinHandle<usize>> = Vec::new();

    //create threads per Partition
    for y in 0..num_partitions{
        let v3 = xss[y].clone();
        tp.push(thread::spawn(move || map_data(&v3)));
    }

    //collect the intermediate sums from all the threads
    let mut intermediate_sums2 : Vec<usize> = Vec::new();
    for q in tp.into_iter(){
        intermediate_sums2.push(q.join().unwrap());
    }
...
fn partition_data(num_partitions: usize, v: &Vec<usize>) -> Vec<Vec<usize>>{
    ...
    //if the number of elements is devidable by num_partitions, then it is easy. If not, we split the remainder to the first n groups.
	if v.len()%num_partitions == 0{
        let partition_size = v.len() / num_partitions;
        let mut l = 0;  //starting point of a group
        let mut r = partition_size;  //ending point of a group
        
        // push vectors of integers to xs
        for _d in 0..num_partitions{
    	    // Create a vector of integers
    	    let mut x1 : Vec<usize> = Vec::new();
    	    // Add the corresponding integers belongs to the group to x1
    	    for i in l..r{
        	    x1.push(v[i]);
    	    }
            //set the boundaries for the next group
    	    l = r;
    	    r = r+partition_size;
    	    // Add x1 to the vector that will be returned by this function
    	    xs.push(x1);
    	...
    	let rema = v.len()%num_partitions;  
    	let mut l = 0;
        let mut r = (v.len() - rema)/num_partitions + 1;
        let mut counter = 0;  //this will record how many 1s we have taken from the remainder
        let mut change_flag = false;  //this flag will set once the remainder has been taken care of
        
        // push vectors of integers to xs
        for _d in 0..num_partitions{
            let partition_size : usize;
            //if we still have remainder left, then the group size should be 1 more than the smaller groups
            if counter < rema{
                partition_size = (v.len() - rema)/num_partitions + 1;
            }
            else{
                partition_size = (v.len() - rema)/num_partitions;
                //this block is used for the transaction from spliting remainder to no remainder
    	    ...
    	    // Create one vector of integers
    	    let mut x1 : Vec<usize> = Vec::new();
    	    // populating the corresponding group
    	    for i in l..r{
        	    x1.push(v[i]);
    	    }
            //set boundaries for the next group
    	    l = r;
    	    r = r+partition_size;
    	    // Add x1 to the vector that will be returned by this function
    	    xs.push(x1);
